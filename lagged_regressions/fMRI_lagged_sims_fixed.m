...
for sims = 1:nsims
    ...
    clearvars -except n_trials nsubs nsims trans_prob prob_market_shuffle eta_values beta_values w_values sims stats_dependent_fixed stats_independent_fixed
...
%             end
    ...
    m1 = fitglme(dat_tab_all((dat_tab_all.correlated_block==1)& (dat_tab_all.forced_trial ==0), :), 'choose_left ~ MB_same_1_back + MB_same_2_back + MB_same_3_back + MB_same_4_back + MB_same_5_back + MB_diff_1_back + MB_diff_2_back + MB_diff_3_back + MB_diff_4_back+ MB_diff_5_back + (1 + MB_same_1_back + MB_same_2_back + MB_same_3_back + MB_same_4_back + MB_same_5_back + MB_diff_1_back + MB_diff_2_back + MB_diff_3_back + MB_diff_4_back+ MB_diff_5_back |this_sub)', 'Distribution', 'binomial'); %dependent blocks
    m2 = fitglme(dat_tab_all((dat_tab_all.correlated_block==-1)& (dat_tab_all.forced_trial ==0), :), 'choose_left ~ MB_same_1_back + MB_same_2_back + MB_same_3_back + MB_same_4_back + MB_same_5_back + MB_diff_1_back + MB_diff_2_back + MB_diff_3_back + MB_diff_4_back+ MB_diff_5_back + (1 + MB_same_1_back + MB_same_2_back + MB_same_3_back + MB_same_4_back + MB_same_5_back + MB_diff_1_back + MB_diff_2_back + MB_diff_3_back + MB_diff_4_back+ MB_diff_5_back |this_sub)', 'Distribution', 'binomial'); %independent blocks

    %save all the relevant resuts in a struct for dependent blocks
    stats_dependent_fixed.stat_comp_estimate(sims, :) = m1.Coefficients.Estimate(2:end)';
    stats_dependent_fixed.stat_comp_tstat(sims, :) = m1.Coefficients.tStat(2:end)';
    stats_dependent_fixed.stat_comp_pval(sims, :) = m1.Coefficients.pValue(2:end)';
    stats_dependent_fixed.stat_SE(sims,:) = m1.Coefficients.SE(2:end);
    
    %save all the relevant results in a struct for independent blocks
    stats_independent_fixed.stat_comp_estimate(sims, :) = m2.Coefficients.Estimate(2:end)';
    stats_independent_fixed.stat_comp_tstat(sims, :) = m2.Coefficients.tStat(2:end)';
    stats_independent_fixed.stat_comp_pval(sims, :) = m2.Coefficients.pValue(2:end)';
    stats_independent_fixed.stat_SE(sims,:) = m2.Coefficients.SE(2:end);
    
end

save 'stats_dependent_fixed.mat'
writematrix(stats_dependent_fixed.stat_comp_estimate, 'stats_dependent_estimates_sims_fixed.csv');

save 'stats_independent_fixed.mat'
writematrix(stats_independent_fixed.stat_comp_estimate, 'stats_independent_estimates_sims_fixed.csv');

toc; %finish measuring how long the code runs for
